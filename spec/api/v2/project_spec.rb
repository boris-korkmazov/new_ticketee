...
describe "/api/v2/projects", :type=> :api do
  ...
  context "projects viewable by this user" do
    ...
    it "json" do
      ...
      projects_json = Project.for(user).to_json(options)
    ...
    it "XML" do
      ...
      expect(last_response.body).to eql(Project.for(user).to_xml(options))

      projects = Nokogiri::XML(last_response.body)
      expect(projects.css("project title").text).to eql(project.name)
